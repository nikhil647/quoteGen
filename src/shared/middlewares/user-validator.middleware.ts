...
export const updateUserValidator = asyncHandler(
  async (req: Request, _: Response, next: NextFunction) => {
    if (!req.params?.id)
    ...
    const [error] = await to(
      updateUserValidationSchema.validateAsync(req.body),
    );
...
export const changePasswordValidator = asyncHandler(
  async (req: Request, _: Response, next: NextFunction) => {
    if (!req.body) throw new BadRequestException("Missing request body!");

    const [error] = await to(
      changePasswordValidationSchema.validateAsync(req.body),
    );
...
export const getUserByIdValidator = asyncHandler(
  async (req: Request, _: Response, next: NextFunction) => {
    if (!req.params?.id)
      throw new BadRequestException('Required parameter "id" is missing!');
